...
-- Estado inicial dos filtros quando o jogador ainda nao escolheu nada. O
-- cliente oficial nasce recolhido, mas aqui a Battle List abre com os filtros a
-- vista de proposito: eles cabem em duas linhas de 20px e o acesso direto vale
-- mais que a economia de altura.
local ESCONDER_FILTROS_POR_PADRAO = false

function isHidingFilters()
  local settings = g_settings.getNode('BattleList')
  if not settings or settings['hidingFilters'] == nil then
    return ESCONDER_FILTROS_POR_PADRAO
  end
...
-- Aceita tanto o indice quanto a propria instancia de BattleClass. Durante o
-- configure() a instancia ainda nao foi inserida em battleClasses, entao a
-- busca por indice devolvia nil e a preferencia salva nunca era aplicada.
local function resolveBattle(idOrObject)
  if type(idOrObject) == 'table' then
    return idOrObject
  end
  return battleClasses[idOrObject]
end

function hideFilterPanel(idOrObject)
  local object = resolveBattle(idOrObject)
  if not object then
    return
  end
...
function showFilterPanel(idOrObject)
  local object = resolveBattle(idOrObject)
  if not object then
    return
  end
