...
-- Lives beside the NPC dialogue, never in the sidebar: trade only ever opens
-- from a conversation, and the official client docks it to the dialogue too.
-- game_npcdialog owns its placement, through dockTradeWindow().
function init()
  npcWindow = g_ui.loadUI('npctrade', rootWidget)
  npcWindow:show()
...
function show()
  if g_game.isOnline() then
    ...
    if modules.game_npcdialog then
      modules.game_npcdialog.dockTradeWindow()
    end
  end
end
...
function hide()
  ...
  -- Lets the dialogue shrink back to its chat-only width.
  if modules.game_npcdialog then
    modules.game_npcdialog.dockTradeWindow()
  end
