...
-- Buttons dropped from the client still sit in already-saved configs, so they are
-- filtered on the way out instead of only from the shipped defaults.
local retiredSideButtons = {manageShortcuts = true}

local function withoutRetired(list)
  local kept = {}
  for _, id in ipairs(list or {}) do
    if not retiredSideButtons[id] then
...
function Options.getActiveWidgets()
  return withoutRetired(Options.array["controlButtonsOptions"]["enabledButtons"])
end

function Options.getInactiveWidgets()
  return withoutRetired(Options.array["controlButtonsOptions"]["disabledButtons"])
end
