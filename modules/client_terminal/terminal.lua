...
-- Whether Ctrl+T opens the terminal. The module itself always loads (it is what
-- captures g_logger output); this only governs the shortcut, so the terminal
-- never reaches a player by accident.
--
-- DEV_TERMINAL is set per developer in config.lua, which is not versioned. It is
-- separate from DEVELOPERMODE on purpose: that one also brings back the
-- Console/Draw/Debug buttons and the profiler, and wanting to read the log is not
-- a reason to change what is on screen.
--
-- Read through a function rather than cached in a local because config.lua is
-- executed before the modules load, but relying on that ordering silently breaks
-- the day it changes.
function isShortcutEnabled()
  return DEVELOPERMODE == true or DEV_TERMINAL == true
end
