...
-- Explored area, kept in the PhysFS write dir (AppData) and read back on the
-- next session so the minimap remembers everything the player has walked.
-- data/minimap.otmm is deliberately not loaded: its blocks sit at x/y under
-- 6336 while this world runs around 32000, so it can never overlap the map.
local PLAYER_OTMM = '/minimap.otmm'
local AUTOSAVE_INTERVAL = 5 * 60 * 1000
...
function terminate()
  ...
  -- Covers closing the client straight from the game, where terminate runs but
  -- onGameEnd may not.
...
function online()
  ...
  -- That priming call goes through the same handler as a real step, so the
  -- dirty flag it raised has to be dropped before the session starts.
  mapDirty = false
  ...
  removeEvent(autosaveEvent)
  autosaveEvent = cycleEvent(saveMinimap, AUTOSAVE_INTERVAL)
end

-- Writes only when something was actually explored, so idling at the character
-- list or a crash-free logout never rewrites the file for nothing.
function saveMinimap()
  if not mapDirty then
    return
  end

  local benchmark = g_clock.millis()
  mapDirty = false
  g_minimap.saveOtmm(PLAYER_OTMM)
  consoleln("Minimap saved in " .. (g_clock.millis() - benchmark) / 1000 .. " seconds.")
end
...
function loadMap(clean)
  ...
  if not MinimapLoader.otmmLoaded then
    if g_resources.fileExists(PLAYER_OTMM) then
      g_minimap.loadOtmm(PLAYER_OTMM)
    end
...
function updateCameraPosition(newPosition, lastPosition)
  ...
  -- The minimap only ever gains tiles by walking, so movement is what marks it
  -- worth saving.
  mapDirty = true
