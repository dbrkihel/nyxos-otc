...
-- Toggles tracking, so the two branches are exactly the official client's
-- QUEST_TRACKER_REMOVE and QUEST_TRACKER_ADD.
function setTrackedMission(missionId)
    ...
    for index, quest in ipairs(questOptions["trackedQuests"]) do
        if quest.id == missionId then
            ...
            if sounds then sounds.playUi(sounds.UiSound.QUEST_TRACKER_REMOVE) end
            return
        end
    end
    table.insert(questOptions["trackedQuests"], {id = missionId, isPinned = false})
    if sounds then sounds.playUi(sounds.UiSound.QUEST_TRACKER_ADD) end
end
